...
const applyMigrations = async (db: SQLite.SQLiteDatabase) => {
    // PRAGMA statements should be outside of a transaction
    // journal_mode = WAL is key for handling "database is locked" errors in concurrent environments
    await db.execAsync('PRAGMA journal_mode = WAL; PRAGMA foreign_keys = ON;');
    
    await db.execAsync('CREATE TABLE IF NOT EXISTS schema_version (version INTEGER PRIMARY KEY, applied_at INTEGER NOT NULL);');
    ...
    if (currentVersion < 1) {
        await db.withTransactionAsync(async () => {
            ...
            await db.execAsync(`
                ...
                CREATE INDEX IF NOT EXISTS idx_api_cache_query ON api_cache(query, source);
                CREATE INDEX IF NOT EXISTS idx_api_cache_expires ON api_cache(expires_at);

                -- Table: helper_pairing
...
export const openDatabase = async (): Promise<SQLite.SQLiteDatabase> => {
    ...
    initializationPromise = (async () => {
        try {
            // openDatabaseAsync returns a shared instance if called with the same name.
            const db = await SQLite.openDatabaseAsync(DB_NAME);
            await applyMigrations(db);
            dbInstance = db;
            console.log('Database connection opened and initialized successfully.');
            return db;
        } catch (error) {
            console.error('Failed to open/initialize database:', error);
